...
public class BenchmarkDbET {
    ...
    public void test() throws Exception {
        ...
        Set<String> ids0 = handleBenchmark(0, market);
        log.info("Size {}", ids0.size());
        for (int i = 0, batchSize = 262144; i < 5; i++, batchSize *= 2) {
            Set<String> ids = handleBenchmark(batchSize, market);
            log.info("Size {}", ids.size());
            //assertEquals(ids0.size(), ids.size());
        }
    }
