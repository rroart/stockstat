...
public class ComponentMLATR extends ComponentML {
    ...
    public ComponentData handle(Market market, ComponentData componentparam, ProfitData profitdata, List<Integer> positions, boolean evolve, Map<String, Object> aMap) {
        ...
        MLATRData param = new MLATRData(componentparam);
        ...
        String localMl = param.getInput().getConfig().getFindProfitMLATRMLConfig();
        ...
        if (param.getInput().getConfig().wantEvolveML()) {
            ComponentMLATR.setnns(param.getService().conf, param.getInput().getConfig(), mlConfigMap, true);
            Map<String, Object> anUpdateMap = param.getService().getEvolveML(true, new ArrayList<>(), PipelineConstants.MLATR, param.getService().conf);
            if (param.getInput().getValuemap() != null) {
                param.getInput().getValuemap().putAll(anUpdateMap); 
            }
        }
        ComponentMLATR.setnns(param.getService().conf, param.getInput().getConfig(), mlConfigMap, false);
        //resultMaps = srv.getContent();
        Map mlMACDMaps = (Map) param.getResultMap(PipelineConstants.MLATR, new HashMap<>());
        //Map mlMACDMaps = (Map) resultMaps.get(PipelineConstants.MLATR);
    ...
    public void calculateIncDec(ComponentData componentparam, ProfitData profitdata, List<Integer> positions) {
        MLATRData param = (MLATRData) componentparam;
        ...
        for (List meta : param.getResultMetaArray()) {
            ...
            if (positions == null || positions.contains(count)) {
                ...
                keys[0] = PipelineConstants.MLATR;
                keys[1] = count;
    ...
    public ComponentData improve(ComponentData componentparam, Market market, ProfitData profitdata, List<Integer> positions, Boolean buy) {
        ...
        ConfigMapChromosome chromosome = new MLATRChromosome(param, profitdata, confList, market, positions, PipelineConstants.MLATR, buy);
        ...
        try {
            ...
            List<MemoryItem> memories = new MLService().doMLATR(new ComponentInput(marketName, null, null, false, false), confMap);
    ...
    public List<MemoryItem> calculateMemory(ComponentData componentparam) throws Exception {
        ...
        for (List meta : param.getResultMetaArray()) {
            ...
            memory.setComponent(PipelineConstants.MLATR);
