...
public class MyHazelcastObjectLock extends MyObjectLock {
    ...
    public MyHazelcastObjectLock() {
        super();
        // for json
    }
    
    public MyHazelcastObjectLock(String path, CuratorFramework curatorClient) {
        super();
        this.path = new MyObjectLockData(path);
        this.curatorClient = curatorClient;
        RetryPolicy retryPolicy = new ExponentialBackoffRetry(1000, 3); 
        this.lock = new DistributedAtomicValue(curatorClient, "/" + Constants.STOCKSTAT + "/" + Constants.DB + "/" + path, retryPolicy);
    }
