...
module "ecs" {
  ...
  availability_zones = local.production_availability_zones
  repository_name_pytorch = "pytorch/production"
  repository_name_tensorflow = "tensorflow/production"
  ...
  MYDBSERVERLOCAL             = var.MYDBSERVERLOCAL
}

module "eks" {
  source             = "./modules/eks"
  #default_sg_id      = module.net.default_sg_id
  cluster-name       = var.cluster-name
    #subnets      = "${var.subnets}"
  environment        = "production"
  vpc_id             = module.net.vpc_id
