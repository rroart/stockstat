...
public class MLRSI extends IndicatorAggregator {
    ...
    private abstract class RsiSubType extends SubType {
    ...
    private class SubTypeRSI extends RsiSubType {
        public SubTypeRSI(Object list, Object taObject, Object resultObject, AfterBeforeLimit afterbefore, int[] range, MyMyConfig conf) {
            ...
            this.filters = new Filter[] { new Filter(true, high, pos), new Filter(false, low, neg) };
        }
        @Override
    ...
    private void makeWantedSubTypes(AbstractCategory cat, AfterBeforeLimit afterbefore) {
        if (conf.wantMLRSI()) {
            if (conf.isRSIEnabled()) {
                ...
                wantedSubTypes.add(new SubTypeRSI(list, taObject, resultObject, afterbefore, TaConstants.ONERANGE, conf));
            }
            if (conf.isSTOCHRSIEnabled()) {
                ...
                wantedSubTypes.add(new SubTypeSRSI(list, taObject, resultObject, afterbefore, TaConstants.ONERANGE, conf));
            }
        }
    }
    ...
    private void calculateMomentums(MyMyConfig conf, Map<String, PeriodData> periodDataMap,
        ...
        doSomething(conf, mapMap, null, null, null, afterbefore);
        ...
        String nnconfigString = conf.getMLRSIMLConfig();
    ...
    private void doLearnTestClassify(NeuralNetConfigs nnConfigs, MyMyConfig conf, Map<String, Map<double[], Double>> mapMap,
        ...
        AfterBeforeLimit afterbefore = new AfterBeforeLimit(conf.getMLRSIDaysBeforeLimit(), conf.getMLRSIDaysAfterLimit());
        // map from h/m + posnegcom to map<model, results>
        int testCount = 0;
        try {
            for (SubType subType : subTypes) {
                Map<String, Map<String, double[]>> mapIdMap = getNewestPosNeg(labelMapShort, TaConstants.ONERANGE, afterbefore, subType.taMap);
                ...
                for (MLClassifyDao mldao : mldaos) {
                    ...
                    for (MLClassifyModel model : mldao.getModels()) {
                        for (int mapTypeInt : getMapTypeList()) {
                            ...
                            LearnTestClassifyResult result = mldao.learntestclassify(nnConfigs, this, map, model, conf.getMLRSIDaysBeforeLimit(), key, mapName, outcomes, mapTime, map2, labelMapShort);  
                ...
                for (MLClassifyDao mldao : mldaos) {
                    ...
                    for (MLClassifyModel model : mldao.getModels()) {
                        ...
                        for (int mapTypeInt : getMapTypeList()) {
                            ...
                            outcomes = 4;
                            log.debug("Outcomes {}", outcomes);
                            Callable callable = new MLClassifyLearnTestPredictCallable(nnConfigs, mldao, this, map, model, conf.getMLRSIDaysBeforeLimit(), key, mapName, outcomes, mapTime, map2, labelMapShort);  
    ...
    private void doLearnTestClassifyOld(NeuralNetConfigs nnconfigs, MyMyConfig conf, Map<String, Map<double[], Double>> mapMap,
        ...
        AfterBeforeLimit afterbefore = new AfterBeforeLimit(conf.getMLRSIDaysBeforeLimit(), conf.getMLRSIDaysAfterLimit());
        Map<String, Map<String, double[]>> mapIdMap = getNewestPosNeg(labelMapShort, TaConstants.ONERANGE, afterbefore, null);
    ...
    private Map<String, Double[]> doClassifications(MyMyConfig conf, Map<String, Map<double[], Double>> mapMap,
        ...
        outcomes = 4;
        log.debug("Outcomes {}", outcomes);
        Map<String, Double[]> classifyResult = mldao.classify(this, map, model, conf.getMLRSIDaysBeforeLimit(), key, mapName, outcomes, labelMapShort, mapTime);
        mapResult2.put(mapType, classifyResult);
            ...
            for (MLClassifyDao mldao : mldaos) {
                for (MLClassifyModel model : mldao.getModels()) {
                    for (int mapTypeInt : getMapTypeList()) {
                        ...
                        outcomes = 4;
                        log.debug("Outcomes {}", outcomes);
                        Double testaccuracy = mldao.learntest(nnConfigs, this, map, model, conf.getMLRSIDaysBeforeLimit(), key, mapName, outcomes, mapTime);  
