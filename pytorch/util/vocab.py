...
class Vocab:
  def __init__(self, counter, specials=[PAD_TOKEN, UNK_TOKEN, BOS_TOKEN, EOS_TOKEN, MASK_TOKEN], unk_token=UNK_TOKEN):
    ...
    i = self.index
    for c in counter:
      self.vocab_dict[str(c)] = i
      self.vocab.append(str(c))
      i = i + 1
    self.index = i
    ...
    for token in self.specials:
      self.vocab_dict[str(token)] = i
      self.vocab.append(str(token))
      i = i + 1
    self.index = i

    #print("dict", self.vocab_dict)
    
    #if unk_token in specials:
    #  self.vocab_dict.set_default_index(self.vocab.get_stoi()[unk_token])
  ...
  # not used
  def to_s(self, idx):
    if idx >= len(self.vocab_dict):
      return UNK_TOKEN
    else:
      return self.vocab_dict.get_itos()[idx]
  ...
  def encode(self, seq):
    ...
    l = []
    for s in seq:
      val = self.vocab_dict.get(s)
      if val is None:
        self.index = self.index + 1
        val = self.index
        self.vocab_dict[str(s)] = val
        self.vocab.append(str(s))
    l.append(val)
    return l
