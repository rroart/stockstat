...
CODE_TYPE_MAP = mmt.utils.inverse_dict(TYPE_CODE_MAP)

# Beat
BEAT_CODE_MAP = {i: i + 1 for i in range(MAX_BEAT + 1)}
BEAT_CODE_MAP[None] = 0
CODE_BEAT_MAP = mmt.utils.inverse_dict(BEAT_CODE_MAP)

# Position
POSITION_CODE_MAP = {i: i + 1 for i in range(RESOLUTION)}
POSITION_CODE_MAP[None] = 0
CODE_POSITION_MAP = mmt.utils.inverse_dict(POSITION_CODE_MAP)

# Pitch
PITCH_CODE_MAP = {i: i + 1 for i in range(128)}
PITCH_CODE_MAP[None] = 0
CODE_PITCH_MAP = mmt.utils.inverse_dict(PITCH_CODE_MAP)
...
CODE_INSTRUMENT_MAP = mmt.utils.inverse_dict(INSTRUMENT_CODE_MAP)
...
def load_encoding(filename):
    ...
    encoding = mmt.utils.load_json(filename)
    for key in (
        "code_type_map",
        "code_beat_map",
        "code_position_map",
...
def main():
    ...
    mmt.utils.save_json(filename, encoding)
