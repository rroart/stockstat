...
def parse_args(dir, args=None, namespace=None):
    ...
    parser.add_argument(
        "-n",
        "--names",
        default=dir + "lmd_full/original-names.txt",
        type=pathlib.Path,
        help="input names",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-o",
        "--out_dir",
        default=dir + "lmd_full/processed/json/",
        type=pathlib.Path,
...
def convert(name, in_dir, out_dir, resolution, skip_existing):
    ...
    out_name = out_name.replace(".mid", "") # TODO why?
...
def main(dir, args):
    ...
    mmt.utils.save_txt(out_filename, converted_names)
